...
sub create_multiple_rows {
	my ($pvrow, $diseaseh) =@_;
	my @a = (); my @hr = ();
		my @keys = qw (relay_id date relay_drug relay_company relay_ind has_child  relay_toplevelind relay_phase pv_tracked_comp pv_other_comp pv_brand pv_generic pv_researchcode);
